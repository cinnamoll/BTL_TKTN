...
dir.create("Code/BTL_TKTN/results/baseline", recursive = TRUE, showWarnings = FALSE)

df_baseline <- data.frame(model = 'Baseline Random Forest (M)', f1_score = f1Score)
write.csv(df_baseline, 'Code/BTL_TKTN/results/baseline/baseline_results.csv', row.names = FALSE)
...
write.csv(df_importance, 'Code/BTL_TKTN/results/baseline/feature_importances.csv', row.names = FALSE)


file_conn <- file("Code/BTL_TKTN/results/baseline/dropped.txt", open="w")
